...
import {SettingsService, LoginDialogComponent, LoginServiceClass, SETTINGS_SERVICE, ZAC_LOGIN_SERVICE, SettingsServiceClass, GrowlerService} from "zt-console-lib";
            ...
            this.dialogRef.afterClosed().pipe(switchMap((result: any) => {
                if (result?.returnToLogin) {
                    ...
                    if (this.settingsService?.settings?.session) {
                        localStorage.removeItem('zt.settings');
