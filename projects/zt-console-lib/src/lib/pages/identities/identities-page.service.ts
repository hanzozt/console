...
export class IdentitiesPageService extends ListPageServiceClass {
    ...
    public resetMFA(identity) {
        return this.ztService.resetMFA(identity.id).then(() => {
            ...
            const message = this.ztService.getErrorMessage(error);
            const growlerData = new GrowlerModel(
                'error',
                'Error',
                `Failed to Reset MFA`,
    ...
    downloadJWT(jwt, name) {
        ...
        element.setAttribute('href', 'data:application/zt-jwt;charset=utf-8,' + encodeURIComponent(jwt));
