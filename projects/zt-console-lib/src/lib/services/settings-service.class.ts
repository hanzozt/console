...
export abstract class SettingsServiceClass {
    ...
    public get() {
        const tmp = localStorage.getItem('zt.settings');
        if (tmp) {
            this.settings = JSON.parse(tmp);
        } else {
            this.settings = {...DEFAULTS};
            localStorage.setItem('zt.settings', JSON.stringify(this.settings));
        }
    ...
    public set(data: any) {
        this.settings = data;
        localStorage.setItem('zt.settings', JSON.stringify(data));
        this.settingsChange.next(this.settings);
    }

    public version() {
        this.versionData = localStorage.getItem('zt.version');
