...
export class ConfigurationFormComponent extends ProjectableForm implements OnInit, OnDestroy {
    ...
    copyCLICommand() {
        const command = `zt edge ${this.formData.id ? 'update' : 'create'} config ${this.formData.id ? `'${this.formData.id}'` : ''} ${this.formData.id ? '--name' : ''} '${this.formData.name}' --data '${JSON.stringify(this.formData.data)}' --tags-json '${JSON.stringify(this.formData.tags)}'`;
