...
export class JwtSignerFormComponent extends ProjectableForm implements OnInit, OnDestroy {
    ...
    get showTokenType() {
        return !isEmpty(this.settingsService?.ztSemver) && semver.gte(this.settingsService?.ztSemver, '1.4.0');
    }
    ...
    copyCLICommand() {
        const command = `zt edge ${this.formData.id ? 'update' : 'create'} ext-jwt-signer ${this.formData.id ? `'${this.formData.id}'` : ''} ${this.formData.id ? '--name' : ''} '${this.formData.name}' ${this.formData.id ? '--issuer' : ''} '${this.formData.issuer}' --audience '${this.formData.audience}' --cert-pem '${this.formData.certPem}' --claims-property '${this.formData.claimsProperty}' --external-auth-url '${this.formData.externalAuthUrl}' --kid '${this.formData.kid}'`;
