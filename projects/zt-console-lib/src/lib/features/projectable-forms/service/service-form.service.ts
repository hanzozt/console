...
export class ServiceFormService {
    ...
    save(formData): Promise<any> {
        ...
        let prom;
        if (isUpdate) {
            prom = this.ztService.patch('services', data, formData.id, true);
        } else {
            prom = this.ztService.post('services', data, true);
        }

        return prom.then(async (result: any) => {
            const id = isUpdate ? formData.id : (result?.data?.id || result?.id);
            let svc = await this.ztService.getSubdata('services', id, '').then((svcData) => {
                return svcData.data;
            });
    ...
    getAssociatedServicePolicies(nameFilter?) {
        ...
        this.ztService.getSubdata('services', this.formData.id, 'service-policies', paging, undefined, filters).then((result: any) => {
    ...
    getAssociatedTerminators(nameFilter?) {
        ...
        this.ztService.getSubdata('services', this.formData.id, 'terminators', paging, undefined, filters).then((result: any) => {
    ...
    addTerminators(serviceId) {
        ...
        this.addedTerminators.forEach((terminator) => {
            ...
            promises.push(this.ztService.post('terminators', terminator));
        });
    }
    ...
    getConfigTypes() {
        this.ztService.get('config-types', this.paging, []).then((result: any) => {
    ...
    getConfigs(filters = [], page = 1) {
        ...
        return this.ztService.get('configs', paging, filters).then((result: any) => {
    ...
    getRouters() {
        this.ztService.get('edge-routers', this.paging, []).then((result: any) => {
    ...
    getAssociatedConfigs(nameFilter?) {
        ...
        this.ztService.getSubdata('services', this.formData.id, 'configs').then((result: any) => {
    ...
    createConfig(configData) {
        return this.ztService.post('configs', configData, true).then((result) => {
            return result;
        }).catch((response) => {
            let msg;
    ...
    async configChanged(resetData = true) {
        ...
        if (this.selectedConfigId === 'preview') {
            this.selectedSchema = await this.ztService.schema(this.selectedConfigType.schema);
