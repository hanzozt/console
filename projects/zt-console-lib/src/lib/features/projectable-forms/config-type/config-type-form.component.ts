...
export class ConfigTypeFormComponent extends ProjectableForm implements OnInit, OnDestroy {
    ...
    copyCLICommand() {
        const command = `zt edge ${this.formData.id ? 'update' : 'create'} config-type ${this.formData.id ? `'${this.formData.id}'` : ''} ${this.formData.id ? '--name' : ''} '${this.formData.name}' --schema '${JSON.stringify(this.formData.schema)}' --tags-json '${JSON.stringify(this.formData.tags)}'`;
    ...
    get saveDisabled() {
        const reservedConfigTypes = ['host.v1', 'host.v2', 'intercept.v1', 'zt-tunneler-client.v1', 'zt-tunneler-server.v1'];
