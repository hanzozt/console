...
export class IdentityFormComponent extends ProjectableForm implements OnInit, OnChanges, AfterViewInit {
  ...
  getAssociatedServices() {
    this.ztService.getSubdata('identities', this.formData.id, 'services').then((result: any) => {
  ...
  getAssociatedServicePolicies() {
    this.ztService.getSubdata('identities', this.formData.id, 'service-policies').then((result: any) => {
  ...
  getCertificateAuthorities() {
    ...
    this.ztService.get('cas', paging, []).then((result: any) => {
  ...
  getAuthPolicies() {
    ...
    this.ztService.get('auth-policies', paging, []).then((result: any) => {
  ...
  disableIdentity() {
    ...
    this.dialogRef.afterClosed().subscribe({
      next: (result) => {
        ...
        this.ztService.post(`/identities/${this.formData.id}/disable`, {durationMinutes: minutesDifference}).then((result) => {
          ...
          const errorMessage = this.ztService.getErrorMessage(error);
          const growlerData = new GrowlerModel(
              'error',
              'Error',
              `Failed to Disabled`,
  ...
  enableIdentity() {
    ...
    this.dialogRef.afterClosed().subscribe({
      next: (result) => {
        ...
        this.ztService.post(`/identities/${this.formData.id}/enable`, {durationMinutes: 0}).then((result) => {
          const growlerData = new GrowlerModel(
              'success',
              'Success',
              `Identity Enabled`,
