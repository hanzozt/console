...
export class PostureCheckFormComponent extends ProjectableForm implements OnInit, OnDestroy {
    ...
    getPostureCheckTypes() {
        return this.ztService.get('posture-check-types', this.ztService.DEFAULT_PAGING, []).then((result) => {
    ...
    copyCLICommand() {
        const command = `zt edge ${this.formData.id ? 'update' : 'create'} posture-check ${this.formData.id ? `'${this.formData.id}'` : ''} ${this.formData.id ? '--name' : ''} '${this.formData.name}' --schema '${JSON.stringify(this.formData.schema)}' --tags-json '${JSON.stringify(this.formData.tags)}'`;
