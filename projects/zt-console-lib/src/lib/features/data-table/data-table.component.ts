...
export class DataTableComponent implements OnChanges, OnInit {
  ...
  private selectColumnDefinition = {
    colId: 'zt-ag-selected',
    field: 'zt-ag-selected',
    suppressSizeToFit: true,
    lockPosition: true,
    suppressMovable: true,
    resizable: false,
    ...
    cellRendererParams: {
      toggleItem: (item) => {
        ...
        this.svc.gridObj.api.ztAllToggled = _.every(this.rowData, {selected: true});
        _.defer(() => {
  ...
  _initGridOptions() {
    this.gridOptions = {
      ...
      onGridReady: (grid) => {
        grid.api.ztAllToggled = false;
        grid.api.ztHideColumn = this.setColumnVisibilityColumn.bind(this);
        grid.api.validateTable = this._validateTable.bind(this);
        grid.api.view = this._view;
        grid.api.ztRowData = this.rowData;
  ...
  _applyColumnState() {
    const tableStateCookie = localStorage.getItem(`zt_${this.svc.tableId}_table_state`);
  ...
  _setColumnWidths() {
    ...
    const colWidthsCookie = localStorage.getItem(`zt_${this.svc.tableId}_column_widths`);
